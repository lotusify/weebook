...
def scrape_vpphongha_office_supplies():
    """Scrape office supplies from VPP Hong Ha"""
    print("Scraping office supplies from VPP Hong Ha...")
    products = []
    
    urls = [
        "https://vpphongha.vn/collections/van-phong-pham",
        "https://vpphongha.vn/collections/van-phong-pham?page=2",
        "https://vpphongha.vn/collections/van-phong-pham?page=3"
    ]
    ...
    for url in urls:
        try:
            ...
            if not products_found:
                ...
                product_links = [link for link in links if '/san-pham/' in link.get('href', '')]
                print(f"Found {len(product_links)} product links")
                
                # Extract basic info from links
                for link in product_links[:10]:  # Limit to 10 per category
                    try:
                        ...
                        if title and len(title) > 3:
                            ...
                            if image_url and image_url.startswith('/'):
                                image_url = 'https://vpphongha.com' + image_url
                ...
                for product in products_found[:10]:  # Limit to 10 per category
                    try:
                        ...
                        if not title:
                            # Try other title selectors based on debug results
                        ...
                        # Extract price based on debug results
...
def main():
    ...
    print("Starting VPP Hong Ha office supplies scraping...")
    
    # Scrape office supplies
    office_products = scrape_vpphongha_office_supplies()
    ...
    # Save to JSON
    with open('vpphongha_office_supplies.json', 'w', encoding='utf-8') as f:
        json.dump(office_products, f, ensure_ascii=False, indent=2)
    
    print(f"Saved {len(office_products)} items to vpphongha_office_supplies.json")
